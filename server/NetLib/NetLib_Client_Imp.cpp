...
{		
	...
	while (! failed_data_queue.empty())
	{
		...
		else
		{
			...
			//这时候可能已经在NetLib_Client_Imp的析构函数里了，不用也不能this->shared_from_this()。
...
void NetLib_Client_Imp::disconnected(std::shared_ptr<NetLib_Client_Imp> keep_alive)
{
	...
	else //已经在有重连次数了，这次是重连失败
	{
		...
		theDelegate->ReconnectFailedHandler(this->shared_from_this(), will_continue_reconnect); //这个Handler可能会修改will_continue_reconnect的值
	}
...
{	
	...
	if (m_pending_ops_count == 0 && m_last_error != no_error) //有错误才触发disconnected；没有error但是m_pending_ops_count减为0的情况发生在如udp tcp连接的切换，中间可能有0的情况
	{
		...
		boostioservice.post(boost::bind(&NetLib_Client_Imp::disconnected, this, this->shared_from_this())); //post是为了彻底出锁，不管调用者外面还有没锁。
	}
}
...
{	
	...
	{		
		...
		boostioservice.post(boost::bind(&NetLib_Client_Delegate::ConnectedHandler, theDelegate.get(), this->shared_from_this())); //外面还有锁，所以只能通过post来出锁了
	}
	else //表示自动重连成功
	{		
		...
		boostioservice.post(boost::bind(&NetLib_Client_Delegate::ReconnectedHandler, theDelegate.get(), this->shared_from_this())); //外面还有锁，所以只能通过post来出锁了
	}
...
{
	std::shared_ptr<NetLib_Client_Imp> keep_alive = this->shared_from_this(); //避免在锁外的时候this还健在，等能进锁了this已经释放了。
	boost::lock_guard<boost::recursive_mutex> lock(client_mutex);
