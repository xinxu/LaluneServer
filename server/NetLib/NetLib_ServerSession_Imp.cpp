...
{	
	...
	{
		session_mutex.unlock();
		boostioservice.post(boost::bind(&NetLib_ServerSession_Delegate::SendFailedHandler, theDelegate, this->shared_from_this(), data, pHint)); //Handler必须直接或间接的经过post才能调用，否则有可能导致死锁
	}
}
...
{		
	...
	{		
		session_mutex.unlock();
		boostioservice.post(boost::bind(&NetLib_ServerSession_Imp::SendCopyAsyncFailed, this, this->shared_from_this(), data_copy, pHint)); //Handler必须直接或间接的经过post才能调用，否则有可能导致死锁
	}
}
