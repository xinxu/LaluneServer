		...
		default:
			//转发的
			{
				...
				if (info.size()) //TODO 这里先临时往第一个发了，之后再改。。
				{
					common::HeaderEx ex;
					char* send_buf;
					if (server_type == SERVER_TYPE_VERSION_SERVER || server_type == SERVER_TYPE_ACCOUNT_SERVER || server_type == SERVER_TYPE_AUTO_MATCH_SERVER) //这两个服务是在登陆以前，于是要加上operation_id（否则不知道回给哪个用户）。否则的话是加user_id
					{
